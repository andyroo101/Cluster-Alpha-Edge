...
for (fileId in 1:runs) {
  ...
  # Find edge clusters
  edgeClustersAlpha <- findEdgeClustersConvex(posFileName = sprintf("%03d_full.pos",fileId), 
                                        clusterStatsFile = sprintf("%03d_stats.txt",fileId),
                                        clusterIndexPosFile = sprintf("%03d_cluster.index.pos",fileId),
  ...
  ## Timings ##
  iterAvgTime <- (Sys.time() - StartTime)/(fileId-540)
  # Time remaining = iters remaining * iterAvgTime
  # End time = sys.time + timeRemaining
  print(paste(c("finish time:", as.character(Sys.time()+iterAvgTime*(runs-fileId)))))
  
  # save the data as an RData format
  assign(expId, bind_rows(summaryList))
  save("id008", file="id008_data")
}
